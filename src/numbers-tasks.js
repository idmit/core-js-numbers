...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const amount = x1 * x2 + y1 * y2;
  return Math.acos(amount / (moduleA * moduleB));
}
...
function toNumber(value, def) {
  const num = +value;
  if (typeof num === 'number' && value !== null && !Number.isNaN(num)) {
    return value;
  }
  return def;
}
...
function getSumOfDigits(num) {
  ...
  return numToStr.split('').reduce((acc, el) => acc + +el, 0);
}
